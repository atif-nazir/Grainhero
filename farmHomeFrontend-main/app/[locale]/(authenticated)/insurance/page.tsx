                  ...
                  {claims.map((claim) => {
                    ...
                    return (
                      <TableRow key={claim._id}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            {/* Module 7: Export for Insurance Companies */}
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={async () => {
                                try {
                                  const batchId = claim.batch_affected?.batch_id
                                  if (!batchId) {
                                  ...
                                  const res = await fetch(`${backendUrl}/api/grain-batches/${batchId}/export-insurance?format=efu`, {
                                  ...
                                  if (res.ok) {
                                    ...
                                    a.href = url
                                    a.download = `claim-${claim.claim_number}-efu.json`
                                    a.click()
                                    toast.success('EFU export downloaded')
                                  } else {
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null
                                  const backendUrl = (await import('@/config')).config.backendUrl
                                  const res = await fetch(`${backendUrl}/api/grain-batches/${batchId}/export-insurance?format=adamjee`, {
                                    headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) }
                                  })
                                  if (res.ok) {
                                    const blob = await res.blob()
                                    ...
                                    const a = document.createElement('a')
                                    a.href = url
                                    a.download = `claim-${claim.claim_number}-adamjee.json`
                                    a.click()
                                    toast.success('Adamjee export downloaded')
                                  } else {
                                    toast.error('Failed to export')
                                  }
                                } catch (error) {
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null
                                  const backendUrl = (await import('@/config')).config.backendUrl
                                  const res = await fetch(`${backendUrl}/api/grain-batches/${batchId}/export-insurance?format=ztbl`, {
                                    headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) }
                                  })
                                  if (res.ok) {
                                    const blob = await res.blob()
                                    ...
                                    const a = document.createElement('a')
                                    a.href = url
                                    a.download = `claim-${claim.claim_number}-ztbl.json`
                                    a.click()
                                    toast.success('ZTBL export downloaded')
                                  } else {
                                    toast.error('Failed to export')
                                  }
                                } catch (error) {
      ...
      <Dialog open={showClaimModal} onOpenChange={setShowClaimModal}>
        <DialogContent>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            {/* Module 7: Photo Upload for Damage Confirmation */}
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setClaimPhotos(files)
                  }}
                />
                <label htmlFor="claim-photos" className="cursor-pointer flex flex-col items-center justify-center space-y-2">
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-2">
                    {claimPhotos.map((file, idx) => (
                      <div key={idx} className="relative">
                        ...
                        <button
                          type="button"
                          onClick={() => setClaimPhotos(claimPhotos.filter((_, i) => i !== idx))}
            ...
            <Button onClick={async () => {
              ...
              try {
                ...
                for (const photo of claimPhotos) {
                  ...
                  const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null
                  const backendUrl = (await import('@/config')).config.backendUrl
                  const uploadRes = await fetch(`${backendUrl}/api/insurance/upload-photo`, {
                    method: 'POST',
                    headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) },
                    body: formData
                  })
                ...
                // Then submit claim with photo URLs
                const body = {
                  ...claimForm,
                  amount_claimed: Number(claimForm.amount_claimed),
                  batch_affected: {
                ...
                const res = await api.post('/api/insurance/claims', body)
                if (res.ok) {
                  toast.success('Claim filed with photos')
                  setShowClaimModal(false)
                  setClaimPhotos([])
                  const refreshed = await api.get<{ claims: InsuranceClaim[] }>('/api/insurance/claims?limit=50')
                  if (refreshed.ok && refreshed.data) setClaims(refreshed.data.claims as unknown as InsuranceClaim[])
                ...
                toast.error((e as Error).message || 'Failed to upload photos')
              } finally {
                setUploadingPhotos(false)
              }
            }} disabled={claimSaving || uploadingPhotos}>
              {claimSaving || uploadingPhotos ? "Submitting..." : "Submit Claim with Photos"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
