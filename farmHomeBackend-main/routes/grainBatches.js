...
router.post('/', [
    ...
    [
        ...
        body('grain_type').isIn(['Wheat', 'Rice', 'Maize', 'Corn', 'Barley', 'Sorghum']).withMessage('Invalid grain type'),
        body('quantity_kg').isFloat({ min: 0.1 }).withMessage('Quantity must be positive'),
        body('moisture_content').optional().isFloat({ min: 0, max: 100 }).withMessage('Moisture content must be between 0-100%')
    ]
], async (req, res) => {
    try {
...
router.put('/:id', [
    ...
    try {
        ...
        const allowedUpdates = [
            'variety', 'grade', 'moisture_content', 'protein_content',
            'farmer_name', 'farmer_contact', 'source_location',
            'notes', 'tags', 'insured', 'insurance_policy_number',
            'insurance_value'
        ];
...
router.post('/:id/dispatch', [
    ...
    [
        ...
        body('dispatch_details.vehicle_number').notEmpty().withMessage('Vehicle number is required'),
        body('dispatch_details.driver_name').notEmpty().withMessage('Driver name is required')
    ]
], async (req, res) => {
    try {
        const errors = validationResult(req);
        ...
        if (batch.status !== 'stored') {
            return res.status(400).json({ error: 'Only stored batches can be dispatched' });
        }
 ...
 *       - bearerAuth: []
 */
router.put('/:id/risk-assessment', [
    auth,
    requirePermission('ai.enable'),
    requireTenantAccess,
    param('id').isMongoId().withMessage('Valid batch ID is required'),
    [
        body('risk_score').isFloat({ min: 0, max: 100 }).withMessage('Risk score must be between 0-100'),
    ...
    try {
        ...
        await batch.updateRiskScore(req.body.risk_score, req.body.confidence || 0.8);
...
router.get('/qr/:qr_code', async (req, res) => {
    try {
        const batch = await GrainBatch.findOne({ qr_code: req.params.qr_code })
            ...
            .populate('buyer_id', 'name contact_info')
            .select('batch_id grain_type quantity_kg status intake_date dispatch_details farmer_name risk_score spoilage_label');
