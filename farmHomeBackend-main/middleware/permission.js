const { hasPermission, ROLE_PERMISSIONS } = require('../configs/role-permissions');
...
const requireTenantAccess = (req, res, next) => {
  try {
    ...
    if (req.user.tenant_id && req.user.tenant_id.toString() !== requestTenantId.toString()) {
      return res.status(403).json({ 
...
const requireResourceOwnership = (resourceField = 'created_by') => {
  return (req, res, next) => {
    try {
      ...
      if (!req.resource) {
        return res.status(500).json({ error: 'Resource not found in request context' });
      }
...
const requireManager = requireRole([USER_ROLES.SUPER_ADMIN, USER_ROLES.ADMIN, USER_ROLES.MANAGER]);
const requireTechnician = requireRole([USER_ROLES.SUPER_ADMIN, USER_ROLES.ADMIN, USER_ROLES.MANAGER, USER_ROLES.TECHNICIAN]);
